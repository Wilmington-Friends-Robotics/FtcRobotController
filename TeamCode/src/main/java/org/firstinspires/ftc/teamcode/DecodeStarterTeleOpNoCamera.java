...
public class DecodeStarterTeleOpNoCamera extends OpMode {
    ...
    public void loop() {
        ...
        double denom = Math.max(1.0, Math.abs(fieldForward) + Math.abs(fieldStrafe) + Math.abs(turn));
        double frontLeftPower = (fieldForward + fieldStrafe + turn) / denom;
        double frontRightPower = (fieldForward - fieldStrafe - turn) / denom;
        double backLeftPower = (fieldForward - fieldStrafe + turn) / denom;
        double backRightPower = (fieldForward + fieldStrafe - turn) / denom;
